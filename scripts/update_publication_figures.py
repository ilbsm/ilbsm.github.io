...
FIGURES_DIR = ROOT / "assets" / "images" / "publications"
FIGURES_URL_PREFIX = "/assets/images/publications"
...
def slugify(value: object, fallback: str = "figure") -> str:
...
def extract_pmc_figures(pmcid: str) -> list[dict]:
    ...
    for index, match in enumerate(re.finditer(r"<figure\b(?P<tag>[^>]*)>(?P<body>.*?)</figure>", page, flags=re.I | re.S), start=1):
        tag = match.group("tag")
        ...
        local_img, source_img = cache_pmc_image(src, pmcid, figure_id, index)
        if source_img in seen_images:
            continue
        seen_images.add(source_img)
...
def write_yaml(entries: list[dict]) -> int:
    lines: list[str] = [
        ...
        "# PubMed Central figure images are cached into assets/images/publications/.",
        "",
    ]
    written = 0
...
def compact_work(work: dict) -> dict:
    ...
    return {
        ...
        "topics": [
            {
                ...
                "subfield": {
                    "display_name": ((topic.get("subfield") or {}).get("display_name") or ""),
                },
                "field": {
                    "display_name": ((topic.get("field") or {}).get("display_name") or ""),
                },
                "domain": {
                    "display_name": ((topic.get("domain") or {}).get("display_name") or ""),
                },
            }
        ...
        "keywords": [
            {
                "display_name": keyword.get("display_name") or "",
